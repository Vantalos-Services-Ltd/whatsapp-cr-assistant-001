...
const stageColors: Record<string, string> = {
  ...
  PROFILE_INCOMPLETE: "bg-yellow-100 text-yellow-800 border-yellow-200 dark:bg-yellow-900/20 dark:text-yellow-400",
            ...
            <div className="flex flex-wrap gap-1.5">
              {progressData.missingFields.map((field, idx) => (
                <Badge
                  key={idx}
                  variant="outline"
                  className="text-xs bg-yellow-50 text-yellow-700 border-yellow-200 dark:bg-yellow-900/10 dark:text-yellow-600"
                >
                  Missing: {field}
                </Badge>
              ))}
